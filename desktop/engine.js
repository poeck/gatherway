...
class Engine {
  ...
  update(s, presence, now, enabled = true) {
    ...
    const atBreak = !!locations.away && s.location === locations.away;
    const atDesk = !!locations.available && s.location === locations.available;
    const participantsKnown = Array.isArray(s.participants);
    const participants = s.participants || [];
    const prev = this.previous;
    const existingConversation = !!prev && Array.isArray(prev.participants) && prev.participants.length > 0 && prev.location !== locations.away;
    const visitors = new Set([...(s.deskVisitors || []).map(id => `desk:${id}`), ...participants.map(id => `avatar:${id}`)]);

    for (const [id, alert] of this.alerts) {
      if (now >= alert.expiresAt || (alert.visit && !visitors.has(alert.visit))) {
        effects.push({ type: 'cancel', id, reason: now >= alert.expiresAt ? 'expired' : 'visitor-left' }); this.alerts.delete(id);
      }
    }
    const alert = (kind, sourceId, visit = null) => {
      ...
      const event = { id: randomUUID(), kind, mode, sourceId, visit, createdAt: now, expiresAt: now + 45000 };
    ...
    if (prev) {
      ...
      for (const wave of s.waves || []) if (Array.isArray(prev.waves) && !this.seenWaves.has(wave.id)) alert('wave', wave.fromId);
    }
